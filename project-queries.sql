...
CREATE PROCEDURE DeletePlayer(IN p_PlayerID SMALLINT) BEGIN
DELETE FROM
    Player
WHERE
    PlayerID = p_PlayerID;
...
CREATE PROCEDURE UpdatePlayer(
    ...
    IN p_Weight SMALLINT,
    IN p_HighSchool VARCHAR(40)
) BEGIN DECLARE affected_rows INT;
...
-- ROW_COUNT() returns the number of rows affected by the last statement (it should be just one row in this case)
IF affected_rows = 0 THEN
SELECT
    'No player updated. Check if PlayerID exists.' AS Message;

ELSE
SELECT
    CONCAT('Updated ', affected_rows, ' player(s).') AS Message;
...
CREATE PROCEDURE GetBothTeamStats(IN p_GameID SMALLINT) BEGIN
SELECT
    t.TeamName,
    tg.*
FROM
    TeamGameStatistic tg
...
WHERE
    ...
    AND (
        (
            MONTH(Game.Date) >= 9
            AND YEAR(Game.Date) = p_StartYear
        )
        OR (
            MONTH(Game.Date) <= 5
            AND YEAR(Game.Date) = p_StartYear + 1
        )
    );
...
CREATE PROCEDURE GetPlayerAveragesBetweenDates(
    ...
    IN p_StartDate DATE,
    IN p_EndDate DATE
) BEGIN IF p_StartDate > p_EndDate THEN SIGNAL SQLSTATE '45000' -- custom error handling in case dates are invalid
SET
    MESSAGE_TEXT = 'Start date must be before end date.';
...
CREATE PROCEDURE GetTeamGameHistory(IN p_TeamID SMALLINT) BEGIN
SELECT
    g.GameID,
    g.Date,
    CASE
        -- conditional to determine the opponent based on whether the team was home or away
...
CREATE PROCEDURE GetTeamSeasonStatistics(IN p_TeamID SMALLINT, IN p_StartYear INT) BEGIN
SELECT
    ROUND(AVG(FieldGoalsMade), 2) AS AvgFieldGoalsMade,
    ROUND(AVG(FieldGoalsAttempted), 2) AS AvgFieldGoalsAttempted,
    ROUND(AVG(ThreePointersMade), 2) AS AvgThreePointersMade,
...
CREATE PROCEDURE GetTeamSeasonStatisticsBetweenDates(
    ...
    IN p_StartDate DATE,
    IN p_EndDate DATE
) BEGIN -- Error handling for invalid date range
IF p_StartDate > p_EndDate THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Start date must be before end date.';
...
CREATE PROCEDURE ComparePlayersSeasonAverages(
    IN p_PlayerID1 SMALLINT,
    IN p_PlayerID2 SMALLINT,
    IN p_StartYear INT
) BEGIN
SELECT
    PlayerID,
...
FROM
    ...
    JOIN Game ON PlayerGameStatistic.GameID = Game.GameID
WHERE
    PlayerID IN (p_PlayerID1, p_PlayerID2)
    AND (
        (
            MONTH(Game.Date) >= 9
            AND YEAR(Game.Date) = p_StartYear
...
CREATE PROCEDURE MVPofTheSeason(IN p_SeasonStartYear INT) BEGIN
SELECT
    PlayerID,
...
WHERE
    (
        MONTH(Game.Date) >= 9
        AND YEAR(Game.Date) = p_SeasonStartYear
    )
    OR (
        MONTH(Game.Date) <= 5
        AND YEAR(Game.Date) = p_SeasonStartYear + 1
    )
GROUP BY
    PlayerID
ORDER BY
...
CREATE PROCEDURE TrackPlayerEfficiency(IN p_PlayerID SMALLINT, IN p_StartYear INT) BEGIN
SELECT
    CONCAT(p_StartYear, '-', p_StartYear + 1) AS Season,
    SUM(
        Points + Rebounds + Assists + Steals + Blocks - (FieldGoalsAttempted - FieldGoalsMade) - (FreeThrowsAttempted - FreeThrowsMade) - Turnovers
...
CREATE PROCEDURE CalculatePlayerPIE(IN player_id SMALLINT, IN game_id SMALLINT) BEGIN DECLARE total_game_stats DOUBLE;
...
SELECT
    (
        Points + FieldGoalsMade + FreeThrowsMade - FieldGoalsAttempted - FreeThrowsAttempted + DefensiveRebounds + (OffensiveRebounds / 2) + Assists + Steals + (Blocks / 2) - PersonalFouls - Turnovers
    ) INTO player_stats
FROM
    PlayerGameStatistic
WHERE
...
SELECT
    (
        SUM(TotalPoints) + SUM(FieldGoalsMade) + SUM(FreeThrowsMade) - SUM(FieldGoalsAttempted) - SUM(FreeThrowsAttempted) + SUM(DefensiveRebounds) + (SUM(OffensiveRebounds) / 2) + SUM(Assists) + SUM(Steals) + (SUM(Blocks) / 2) - SUM(PersonalFouls) - SUM(Turnovers)
    ) INTO total_game_stats
FROM
    TeamGameStatistic
WHERE
    GameID = game_id;

IF total_game_stats = 0 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Game statistics cannot be zero.';

ELSE
SELECT
    player_stats / total_game_stats AS PlayerImpactEstimate;

END IF;

END -- Calculate Player Impact Estimate (PIE) for a given season
DROP PROCEDURE IF EXISTS CalculateSeasonPlayerPIE;

CREATE PROCEDURE CalculateSeasonPlayerPIE(
    IN p_PlayerID SMALLINT,
    IN p_StartYear INT
) BEGIN -- Temporary table to store individual game PIE values
...
WHERE
    (
        MONTH(Date) >= 9
        AND YEAR(Date) = p_StartYear
    )
    OR (
        MONTH(Date) <= 5
        AND YEAR(Date) = p_StartYear + 1
    );
...
game_loop: LOOP FETCH cur INTO v_GameID;

IF done THEN LEAVE game_loop;
...
-- Calculate and return the average game PIE for the season
SELECT
    AVG(GamePIE) AS SeasonPlayerImpactEstimate
FROM
    TempPIE;
...
CREATE PROCEDURE GetPlayerSuccessImpact(
    IN p_PlayerID SMALLINT,
    IN p_StartYear INT
) BEGIN DECLARE avg_points DECIMAL(5, 2);
...
SELECT
    COUNT(*) INTO games_won
FROM
    PlayerGameStatistic AS pgs
    JOIN Game AS g ON pgs.GameID = g.GameID
    JOIN TeamGameStatistic AS tgs ON g.GameID = tgs.GameID
WHERE
    pgs.PlayerID = p_PlayerID
    AND pgs.Points > avg_points -- check if the player's team won the game
    AND (
        (
            tgs.TeamID = g.HomeTeamID
            AND g.HomeTeamID = (
                SELECT
                    TeamID
                FROM
            ...
            AND tgs.TotalPoints > (
                ...
                WHERE
                    GameID = g.GameID
                    AND TeamID = g.AwayTeamID
            )
        )
        OR (
            tgs.TeamID = g.AwayTeamID
            AND g.AwayTeamID = (
                SELECT
                    TeamID
                FROM
    ...
    AND (
        (
            MONTH(g.Date) >= 9
            AND YEAR(g.Date) = p_StartYear
        )
        OR (
            MONTH(g.Date) <= 5
            AND YEAR(g.Date) = p_StartYear + 1
        )
    );
...
SELECT
    ...
    ) AS WinningImpactPercentage;

END -- Get Top Scorers From a Specific Game Procedure
...
CREATE PROCEDURE PlayerGameLogProcedure(IN p_PlayerID SMALLINT) BEGIN
SELECT
    p.FirstName,
    p.LastName,
    p.Position,
...
CREATE PROCEDURE SeasonalTeamRankingsProcedure(IN p_StartYear INT) BEGIN
SELECT
    t.TeamName,
    COUNT(
        CASE
            WHEN (
                g.HomeTeamID = t.TeamID
                AND tgs.TotalPoints > ogs.TotalPoints
            )
            OR (
...
FROM
    ...
    JOIN TeamGameStatistic tgs ON g.GameID = tgs.GameID
    AND tgs.TeamID = t.TeamID
    JOIN TeamGameStatistic ogs ON g.GameID = ogs.GameID
    AND ogs.TeamID <> t.TeamID
WHERE
    (
        (
            MONTH(g.Date) >= 9
...
CREATE PROCEDURE VerifyPoints(IN p_GameID SMALLINT, IN p_TeamID SMALLINT) BEGIN DECLARE PlayerPoints INT DEFAULT 0;
...
SELECT
    COALESCE(TotalPoints, 0) INTO TeamTotalPoints
FROM
    TeamGameStatistic
WHERE
    GameID = p_GameID
...
IF PlayerPoints = 0 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = CONCAT(
        'Error: Player points total returns zero or null for TeamID: ',
        p_TeamID,
        ', GameID: ',
        p_GameID
    );
...
IF TeamTotalPoints = 0 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = CONCAT(
        'Error: Team total points return zero or null for TeamID: ',
        p_TeamID,
        ', GameID: ',
        p_GameID
    );

ELSE IF PlayerPoints != TeamTotalPoints THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = CONCAT(
        'Error: Mismatch in points! Player total: ',
        PlayerPoints,
        ' vs Team total: ',
        TeamTotalPoints
    );
...
ELSE
SELECT
    'Points are consistent!' AS Message;
...
CREATE PROCEDURE VerifyAllStats(IN p_GameID SMALLINT, IN p_TeamID SMALLINT) BEGIN DECLARE PlayerFGMade,
PlayerFGAttempted,
Player3PMade,
Player3PAttempted,
PlayerFTMade,
...
SELECT
    SUM(COALESCE(FieldGoalsMade, 0)),
    SUM(COALESCE(FieldGoalsAttempted, 0)),
    SUM(COALESCE(ThreePointersMade, 0)),
    SUM(COALESCE(ThreePointersAttempted, 0)),
    SUM(COALESCE(FreeThrowsMade, 0)),
    SUM(COALESCE(FreeThrowsAttempted, 0)),
    SUM(COALESCE(PersonalFouls, 0)),
    SUM(COALESCE(Rebounds, 0)),
    SUM(COALESCE(OffensiveRebounds, 0)),
    SUM(COALESCE(DefensiveRebounds, 0)),
    SUM(COALESCE(Assists, 0)),
    SUM(COALESCE(Steals, 0)),
    SUM(COALESCE(Blocks, 0)),
    SUM(COALESCE(Turnovers, 0)),
    SUM(COALESCE(Points, 0)) INTO PlayerFGMade,
    PlayerFGAttempted,
    Player3PMade,
    Player3PAttempted,
    PlayerFTMade,
...
SELECT
    COALESCE(FieldGoalsMade, 0),
    COALESCE(FieldGoalsAttempted, 0),
    COALESCE(ThreePointersMade, 0),
    COALESCE(ThreePointersAttempted, 0),
    COALESCE(FreeThrowsMade, 0),
    COALESCE(FreeThrowsAttempted, 0),
    COALESCE(PersonalFouls, 0),
    COALESCE(Rebounds, 0),
    COALESCE(OffensiveRebounds, 0),
    COALESCE(DefensiveRebounds, 0),
    COALESCE(Assists, 0),
    COALESCE(Steals, 0),
    COALESCE(Blocks, 0),
    COALESCE(Turnovers, 0),
    COALESCE(TotalPoints, 0) INTO TeamFGMade,
    TeamFGAttempted,
    Team3PMade,
    Team3PAttempted,
    TeamFTMade,
...
IF PlayerFGMade != TeamFGMade THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Field Goals Made';

ELSEIF PlayerFGAttempted != TeamFGAttempted THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Field Goals Attempted';

ELSEIF Player3PMade != Team3PMade THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Three Pointers Made';

ELSEIF Player3PAttempted != Team3PAttempted THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Three Pointers Attempted';

ELSEIF PlayerFTMade != TeamFTMade THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Free Throws Made';

ELSEIF PlayerFTAttempted != TeamFTAttempted THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Free Throws Attempted';

ELSEIF PlayerFouls != TeamFouls THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Personal Fouls';

ELSEIF PlayerRebounds != TeamRebounds THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Total Rebounds';

ELSEIF PlayerOffRebounds != TeamOffRebounds THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Offensive Rebounds';

ELSEIF PlayerDefRebounds != TeamDefRebounds THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Defensive Rebounds';

ELSEIF PlayerAssists != TeamAssists THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Assists';

ELSEIF PlayerSteals != TeamSteals THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Steals';

ELSEIF PlayerBlocks != TeamBlocks THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Blocks';

ELSEIF PlayerTurnovers != TeamTurnovers THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Turnovers';

ELSEIF PlayerPoints != TeamPoints THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Mismatch in Total Points';

ELSE
SELECT
    'All statistics are consistent!' AS Message;
...
AFTER
    DELETE ON Player FOR EACH ROW BEGIN
INSERT INTO
    DeletionLog (EntityType, EntityID, DeletedAt, DeletedBy)
VALUES
...
AFTER
    DELETE ON Team FOR EACH ROW BEGIN
INSERT INTO
    DeletionLog (EntityType, EntityID, DeletedAt, DeletedBy)
VALUES
...
AFTER
    DELETE ON Game FOR EACH ROW BEGIN
INSERT INTO
    DeletionLog (EntityType, EntityID, DeletedAt, DeletedBy)
VALUES
